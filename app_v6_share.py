...
HOMECC_DESIGN_LIBRARY_HERO = _logo("homecc_lvt_design_library_hero.png")
...
.design-hero-link {{ display:block; position:relative; min-height:520px; border-radius:14px; overflow:hidden; border:1px solid {T['border']}; background-size:cover; background-position:center; text-decoration:none !important; box-shadow:0 24px 80px rgba(0,0,0,.22); margin-bottom:14px; }}
.design-hero-link::before {{ content:""; position:absolute; inset:0; background:linear-gradient(180deg,rgba(7,11,18,.02),rgba(7,11,18,.16) 45%,rgba(7,11,18,.76)); pointer-events:none; }}
.design-hero-link:hover {{ border-color:{GOLD}; filter:brightness(1.04); }}
.design-hero-content {{ position:absolute; left:0; right:0; bottom:0; padding:28px 32px; color:#fff; z-index:1; display:flex; align-items:flex-end; justify-content:space-between; gap:22px; }}
.design-hero-k {{ color:{GOLD}; font-size:11px; font-weight:900; letter-spacing:1.5px; text-transform:uppercase; margin-bottom:9px; }}
.design-hero-t {{ color:#fff; font-size:34px; font-weight:900; line-height:1.08; margin-bottom:9px; }}
.design-hero-d {{ color:rgba(255,255,255,.75); font-size:13px; line-height:1.65; max-width:760px; }}
.design-hero-btn {{ flex:0 0 auto; min-width:168px; text-align:center; padding:13px 18px; border-radius:999px; background:#111; color:#fff; font-size:13px; font-weight:900; box-shadow:0 14px 34px rgba(0,0,0,.30); }}
.design-library-use {{ display:grid; grid-template-columns:repeat(3,minmax(0,1fr)); gap:12px; margin-bottom:12px; }}
...
@media (max-width: 1000px) {{
  ...
  .design-library-use {{ grid-template-columns:1fr; }}
  .design-hero-link {{ min-height:380px; }}
  .design-hero-content {{ flex-direction:column; align-items:flex-start; padding:22px; }}
  .design-hero-t {{ font-size:28px; }}
...
d_newsales = delta_pct(df_newsales, '신규주택판매')
...
# 🧱 DESIGN LIBRARY
# ════════════════════════════════════════════════════════════
elif menu == "🧱 Design Library":
    st.markdown('<div class="sec"><span class="sec-t">Design Library</span><span class="sec-s">HomeCC LVT 공식 디자인 레퍼런스</span><span class="live"><span class="dot"></span>Official</span></div>', unsafe_allow_html=True)

    hero_bg = (
        f"linear-gradient(180deg,rgba(7,11,18,.02),rgba(7,11,18,.12)), url('data:image/png;base64,{HOMECC_DESIGN_LIBRARY_HERO}')"
        if HOMECC_DESIGN_LIBRARY_HERO
        else "linear-gradient(135deg,#11161F,#0E2372)"
    )
    st.markdown(
        f"""
        <a class="design-hero-link" href="{HOMECC_LVT_DESIGN_LIBRARY_URL}" target="_blank" rel="noopener noreferrer" style="background-image:{hero_bg};">
          <div class="design-hero-content">
            <div>
              <div class="design-hero-k">Official HomeCC Reference</div>
              <div class="design-hero-t">LVT Design Library</div>
              <div class="design-hero-d">
                신규 LVT 디자인, 패턴, 컬러 레퍼런스를 공식 페이지에서 바로 확인합니다.
                디자인팀과 영업팀이 같은 자료를 기준으로 고객 미팅 전 제안 방향을 맞출 수 있습니다.
              </div>
            </div>
            <div class="design-hero-btn">View More</div>
          </div>
        </a>
        """,
        unsafe_allow_html=True,
    ...
    st.markdown('<div class="panel"><div class="p-head"><span class="p-t">How to Use</span><span class="p-m">Design · Sales · Product story</span></div><div class="p-body">', unsafe_allow_html=True)
    st.markdown(
        """
        <div class="design-library-use">
          <div class="design-library-card">
            <div class="design-library-card-k">Design Team</div>
            <div class="design-library-card-t">신규 디자인 레퍼런스 확인</div>
            <div class="design-library-card-d">공식 라이브러리에서 최신 패턴과 컬러를 확인하고, 외부 트렌드 키워드와 실제 제품 방향을 매칭합니다.</div>
          </div>
          <div class="design-library-card">
            <div class="design-library-card-k">Sales Meeting</div>
            <div class="design-library-card-t">고객 선호도 기반 빠른 제안</div>
            <div class="design-library-card-d">고객이 선호하는 wood, stone, neutral, texture 계열을 바로 열어보며 미팅 중 레퍼런스로 활용합니다.</div>
          </div>
          <div class="design-library-card">
            <div class="design-library-card-k">Product Story</div>
            <div class="design-library-card-t">시장 트렌드와 공식 디자인 연결</div>
            <div class="design-library-card-d">Design Intelligence에서 본 시장 흐름을 공식 디자인 라이브러리와 연결해 제품 제안 논리를 만듭니다.</div>
          </div>
        </div>
        """,
        unsafe_allow_html=True,
